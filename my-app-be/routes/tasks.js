...
router.post('/', async (req, res) => {
  ...
  try {
    const userIdResult = await client.query("SELECT * FROM users WHERE first_name = $1 AND last_name = $2;", [names[0], names[1]]);
    const userId = userIdResult.rows[0]?.id;
    if (!userId) {
      return res.status(404).json({ error: 'User not found' });
    }

    const project = await client.query('INSERT INTO projects (name, state, final_deadline) VALUES ($1, $2, $3) RETURNING *;', [task, 'in progress', date]);
    const projectId = project.rows[0].id;
    console.log(projectId);
    const taskResult = await client.query('INSERT INTO tasks (decription, state, deadline, project) VALUES ($1, $2, $3, $4) RETURNING *;', [task, 'in progress', date, projectId]);
