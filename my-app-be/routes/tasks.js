...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await client.query('SELECT t.id, t.decription, t.deadline, pm.user_id, pm.project, pm.admin, t.state, p.project_priority FROM tasks t JOIN projectmember pm ON pm.task_id=t.id JOIN projects p ON t.project=p.id WHERE pm.user_id = $1;', [id]);
